...
export default function SupplyChainOfficerDashboard() {
    ...
    if (error) {
      ...
      const processedRequests = (data || []).map(request => {
        ...
        return {
          ...
          requester_name: request.requester_name || `User ${request.requester_id}`,
  ...
  const handleApproveRequest = async (requestId) => {
    try {
      const { error } = await requestService.updateRequestStatus(requestId, 'approved')
  ...
  const handleRejectRequest = async (requestId) => {
    try {
      const { error } = await requestService.updateRequestStatus(requestId, 'rejected')
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center justify-between lg:justify-start lg:space-x-4 w-full lg:w-auto">
            ...
            <div className="text-center flex-1">
              <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 mb-2">Supply Chain Officer Dashboard</h1>
              <p className="text-slate-600 text-sm sm:text-lg">Manage procurement, inventory levels, and supply requests</p>
            </div>
          </div>
      ...
      {lowStockItems > 0 && (
        <Alert>
          ...
          <AlertDescription>
            {lowStockItems} items are below their minimum threshold and need immediate restocking.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-xl sm:text-2xl font-bold">Supply Chain Management</h2>
              ...
              <TableBody>
                {requests.filter(req => req.status === 'pending').map((request) => (
                  <TableRow key={request.id}>
                    ...
                    <TableCell className="px-4">{request.requester_name}</TableCell>
                ...
                {items.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="px-4">{item.min_threshold}</TableCell>
                      <TableCell className="px-4">
